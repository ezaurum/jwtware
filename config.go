...
type Config struct {
	...
	// JwtExtractors are functions to extract token from the request
	// Optional. Default value is nil
	// These functions are executed before TokenLookup
...
func (cfg *Config) getExtractors() []JwtExtractor {
	...
	if len(cfg.Extractors) > 0 {
		extractors = append(extractors, cfg.Extractors...)
	}
